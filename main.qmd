...
A *full-adder* accepts the $C_{out}$ as $C_{in}$. Where it acts as a term.
...
> Look at the inputs. A Full Adder is designed to be chained together, so it needs to handle the carry from the previous position.

1. [ ] A Half Adder produces a Carry output, but a Full Adder does not.
1. [ ] A Full Adder has two inputs (A, B), while a Half Adder has three.
1. [x] A Full Adder has a Carry Input ($C_{in}$), while a Half Adder does not.
1. [ ] A Half Adder is used for subtraction, while a Full Adder is used for addition.

# Half Adder Logic (single choice)

In a Half Adder, if the inputs are $A=1$ and $B=1$, what are the outputs?

> Recall binary addition: $1 + 1 = 10_2$. The LSB is the Sum, the MSB is the Carry.
...
# Ripple-Carry Adder Construction (single choice)

How many **Full Adders** are required to build an **N-bit** Ripple-Carry Adder?

> Each bit position requires one adder to handle the bits $A_i$, $B_i$, and $C_{in}$.
...
# Ripple-Carry Connections (single choice)

In a Ripple-Carry Adder, how are the individual Full Adders connected?

> The "Ripple" name comes from how the carry signal propagates through the chain.

1. [ ] The Sum ($S$) of one adder connects to the Carry In ($C_{in}$) of the next.
1. [ ] The Carry Out ($C_{out}$) of one adder connects to the Carry Out ($C_{out}$) of the next.
1. [x] The Carry Out ($C_{out}$) of one adder connects to the Carry In ($C_{in}$) of the next.
1. [ ] They are not connected; they operate in parallel.

# Hardware Subtraction (single choice)

How is subtraction ($A - B$) typically performed using standard adder hardware?

> We use the property $A - B = A + (-B)$.

1. [ ] By using a distinct "Full Subtractor" module instead of a Full Adder.
1. [x] By adding A to the Two's Complement of B.
1. [ ] By inverting A and adding it to B.
1. [ ] By setting the Carry In ($C_{in}$) to 0.

# ALU Definition (multiple choice)

Which of the following statements accurately describe an **Arithmetic/Logic Unit (ALU)**?

> The ALU is the "calculator" inside the CPU.

- [x] It combines various mathematical (add, sub) and logical (AND, OR) operations into one unit.
- [x] It forms the heart of most computer systems / processors.
- [ ] It is a sequential circuit that stores memory.
- [ ] It is primarily used to generate clock signals.
```
:::
